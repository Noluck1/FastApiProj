...
const rightBranch = ref<HTMLElement | null>(null)
const middleBranch = ref<HTMLElement | null>(null)
const leftBranch = ref<HTMLElement | null>(null)
const isPointerInside = ref(false)
const isFocused = ref(false)
const reducedMotion = usePreferredReducedMotion()
const isInteracting = computed(() => !props.disabled && (isPointerInside.value || isFocused.value))

const branchMotion = [
  { element: rightBranch, base: 10, amplitude: 15, period: 3000, delay: 0 },
  { element: middleBranch, base: 10, amplitude: 5, period: 3000, delay: 1000 },
  { element: leftBranch, base: -5, amplitude: 5, period: 2000, delay: 1000 },
]

let motionStrength = 0
let motionElapsed = 0

const setBranchAngles = (strength: number, elapsed: number) => {
  for (const branch of branchMotion) {
    const activeElapsed = Math.max(0, elapsed - branch.delay)
    const angle =
      branch.base +
      (activeElapsed > 0
        ? Math.sin((activeElapsed / branch.period) * Math.PI * 2) * branch.amplitude * strength
        : 0)

    branch.element.value?.style.setProperty('transform', `rotate(${angle.toFixed(3)}deg)`)
  }
}

const setBranchesToRest = () => {
  motionStrength = 0
  motionElapsed = 0
  setBranchAngles(0, 0)
}
...
const { pause: pauseMotion, resume: resumeMotion } = useRafFn(
  ({ delta }) => {
    ...
    const targetStrength = isInteracting.value ? 1 : 0
    const damping = targetStrength ? 0.0045 : 0.0016

    motionStrength += (targetStrength - motionStrength) * (1 - Math.exp(-frameDelta * damping))
    motionElapsed += frameDelta
    setBranchAngles(motionStrength, motionElapsed)

    if (!isInteracting.value && motionStrength < 0.002) {
      setBranchesToRest()
      pauseMotion()
    }
  },
...
const startMotion = () => {
  if (reducedMotion.value === 'reduce' || props.disabled) return
  resumeMotion()
}
...
const handlePointerLeave = () => {
  ...
  if (!isFocused.value) resumeMotion()
}

const handleFocus = (event: FocusEvent) => {
  isFocused.value = (event.currentTarget as HTMLElement).matches(':focus-visible')
  if (isFocused.value) startMotion()
}

const handleBlur = () => {
  isFocused.value = false
  if (!isPointerInside.value) resumeMotion()
}
...
watch(reducedMotion, (preference) => {
  if (preference !== 'reduce') return
  pauseMotion()
  setBranchesToRest()
})

onUnmounted(pauseMotion)
</script>
...
<template>
  <Button
    ...
    @blur="handleBlur"
  >
    <span class="auth-submit-button__label"><slot /></span>

    <span
      ref="rightBranch"
      class="auth-submit-button__branch auth-submit-button__branch--right"
      aria-hidden="true"
    >
      <svg viewBox="0 0 26.3 65.33" xmlns="http://www.w3.org/2000/svg">
        <path
    ...
    <span
      ref="middleBranch"
      class="auth-submit-button__branch auth-submit-button__branch--middle"
      aria-hidden="true"
    >
      <svg viewBox="0 0 11.67 37.63" xmlns="http://www.w3.org/2000/svg">
        <path
    ...
    <span
      ref="leftBranch"
      class="auth-submit-button__branch auth-submit-button__branch--left"
      aria-hidden="true"
    >
      <svg viewBox="0 0 25.29 76.92" xmlns="http://www.w3.org/2000/svg">
        <path
...
.auth-submit-button::before {
  ...
  opacity: 0;
  pointer-events: none;
  animation: auth-submit-wind 2s ease-in-out infinite;
  animation-play-state: paused;
  transition: opacity 450ms ease;
}

.auth-submit-button:not(:disabled):is(:hover, :focus-visible) {
  border-color: #f3b182;
}

.auth-submit-button:not(:disabled):is(:hover, :focus-visible)::before {
  opacity: 1;
...
.auth-submit-button__branch svg {
  display: block;
  width: 100%;
  height: auto;
  fill: currentColor;
...
.auth-submit-button__branch--right {
  right: 0;
  width: 25px;
...
.auth-submit-button__branch--middle {
  left: 25px;
  width: 12px;
  transform: rotate(10deg);
}

.auth-submit-button__branch--left {
  left: 0;
  width: 18px;
  transform: rotate(-5deg);
}

@keyframes auth-submit-wind {
  0%,
  100% {
