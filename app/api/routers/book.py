...
async def add_book(book: SBooksAdd = Depends()) -> ApiResponseSchema:
    ...
    return success(data={"title": new_book}, message="Book created successfully")
...
async def get_book_id(book_id: int) -> ApiResponseSchema:
    ...
    return success(data={"id":book.id, "title": book.title, "author": book.author}, message="Book retrieved successfully")
...
async def update_book(book_id: int, book: SBooksUpdate = Depends()) -> ApiResponseSchema:
    ...
    return success(data={"id":book.id, "title": book.title, "author": book.author},message="Book deleted successfully")

@router.delete("/{book_id}")
async def delete_book(book_id: int) -> ApiResponseSchema:
    result = await BookService().delete_book(book_id)
    return success(data=result.id,message="Book deleted successfully")